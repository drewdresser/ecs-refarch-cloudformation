...
2. Run the output of that command
3. Create ECR Repositories
4. Put your ECR id in the Makefiles in each service
5. Go to each service: 1) run `make` then test the endpoint. 2) run `make publish` to push to ECR.
6. Update the S3 bucket in `deploy.sh`
7. Run `./deploy.sh`
